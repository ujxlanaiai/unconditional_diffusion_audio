...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # # Check if waveform is not empty
        # if len(waveform) == 0:
        #     print(f"Warning: Empty waveform found for file {audio_file}")
        #     return None
        #
        # # Check if waveform has constant values
        # if np.std(waveform) == 0:
        #     print(f"Warning: Constant waveform found for file {audio_file}")
        #     return None
...
for epoch in range(num_epochs):
    ...
    for batch in tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        if batch is None:
            continue  # Skip this batch if it's NoneType
